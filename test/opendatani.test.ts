...
import { fetchDataOpenDataNI, DATE_FORMAT } from '../src/index';

describe('Open Data NI Integration', () => {
  beforeEach(() => {
    vi.resetAllMocks();
  ...
  it('constructs the correct API URL and formats response properly', async () => {
    ...
    const results = await fetchDataOpenDataNI('test');

    expect(fetch).toHaveBeenCalledWith(
      'https://admin.opendatani.gov.uk/api/3/action/package_search?q=test',
      expect.any(Object)
    );
  ...
  it('handles empty search results gracefully', async () => {
    ...
    const results = await fetchDataOpenDataNI('nonexistent');
  ...
  it('handles API errors gracefully', async () => {
    ...
    await expect(fetchDataOpenDataNI('test')).rejects.toThrow('Network error');
  });
  ...
  it('handles non-200 responses gracefully', async () => {
    ...
    await expect(fetchDataOpenDataNI('test')).rejects.toThrow('Open Data NI API returned 500');
  });
  ...
  it('handles missing data structure gracefully', async () => {
    ...
    await expect(fetchDataOpenDataNI('test')).rejects.toThrow('Invalid response format from Open Data NI API');
  });
}); 